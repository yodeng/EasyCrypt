...
class DecryptData(object):
    ...
    @classmethod
    def decrypt_file(cls, in_filename, out_file=None, chunksize=64*1024, key="G+b8u9XWZljDTy7Lbp/eqdKeAhvC46gl"):
        if out_file is None:
            out_file = in_filename + ".enc"
        with open(out_file, "wb") as fo:
            for line in cls.decrypt_file_iter(in_filename=in_filename, chunksize=chunksize, key=key):
                fo.write(line)

    @classmethod
    def decrypt_file_readline(cls, in_filename, key="G+b8u9XWZljDTy7Lbp/eqdKeAhvC46gl"):
        line = ""
        for src in cls.decrypt_file_iter(in_filename, chunksize=64*1024, key=key):
            res = re.split("\n", src.decode())
            res[0] = line + res[0]
            yield res[0] + "\n"
            if len(res) == 1:
